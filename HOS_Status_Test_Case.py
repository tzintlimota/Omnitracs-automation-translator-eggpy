...
class HOS_Status_Test_Case(object):
    ...
    def clock_in(self, num_min=None, remark1=None, remark2=None, reason=None, click_continue=True, finish=True):
        format = '%H:%M'
        format2 = '%M'
        ...
        if click_continue:
            # Click SAVE button
            self.img_proc.click_image_by_max_key_points('IVG_Common/Login/OKButton/OKButton')
            print('>>>> Click in OK button to continue with Clock In')

            self.img_proc.expect_image('vnc-clock-in-enter-reason-screen', 'ExpectedScreens', 3)
            self.img_proc.click_image_by_max_key_points_offset('ELD_Core/StatusTab/ClockInReason/ReasonForClockIn',
                                                               0, 40)
            # Clock IN reason is entered
            self.img_proc.send_keys(reason)
            ...
            if finish:
                ...
                print(f'>>>> A CLOCK IN has been set with the time {new_time}')
            else:
                print('------Clock In will not be SAVED, currently in REASON FOR CLOCK IN screen...------')
        else:
            print('------Clock In will not be confirmed, currently in CLOCK IN screen...------')

...
origin_pc = inspector_logs[0][0].split()
origin_off = inspector_logs[1][0].split()

# This is to subtract the minutes (time) to (current_time2) to new_time that will be entered.
new_time = datetime.strptime(str(origin_pc[0]), time_format) - datetime.strptime(str(origin_off[0]), time_format)
